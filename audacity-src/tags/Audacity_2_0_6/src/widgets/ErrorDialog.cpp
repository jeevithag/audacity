...
{
   if( dhelpURL.StartsWith(wxT("HelpSystem::innerlink:")) )
   {
      ShowHtmlText(
         this, 
                            ...
                            const bool Close)
{
   ...
   dlog-// This needs to be modeless because user may need to
   // stop playback AND read dialog's instructions.;
   dlog->CentreOnParent();
   dlog->Show();
   // ANSWER-ME: Vigilant Sentry flags this method as not de}
