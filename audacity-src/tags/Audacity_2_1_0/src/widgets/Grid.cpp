...
void ChoiceEditor::BeginEdit(int row, int col, wxGrid* grid)
{
   ...
   Choice()->SetFocus(bool ChoiceEditor::EndEdit(int row, int col, wxGrid *grid)
{
    wxString newvalue;
                           ...
                           const wxString &WXUNUSED(oldval), wxString *newval* grid)
{
   ...
   wxString val = mChoices[selbool changed = val != mOld;
...
wxAccStatus GridAx::GetName(int childId, wxString *name)
{
   ...
   if (GetRowCol(childId, row, col)) {
      ...
      if (c && d && c == d) {
         double value;
         v.ToDouble(&value)NumericTextCtrl tt(NumericConverter::TIME, TextCtrl tt(mGrid,
                         wxID_ANY,
                         c->GetFormat(),
                         value,
                         c->GetRate(),
