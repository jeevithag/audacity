...
BEGIN_EVENT_TABLE(Meter, wxPanel)
   ...
   EVT_MENU(MonitorID, Meter::OnMonitorat)
...
//   mDisabledBkgndBrush = wxBrush(theTheme.Colour( clrMeterDisabledBrush), wxSOLID);
   ...
   mRuler.SetFlip(true);
   mRuler.SetLabelEdges(trueeateIcon(2);
   ...
   // balistics are right for 44KHz and a bit more frisky than they sh}
...
xBitmapUnregister for our preference update event
...
float Meter::GetMaxPeak()
{
   ...
   for(j=0; wxFont Meter::GetFont() constf (mDB)
...
{
   for (int c = 0; c SetActiveStyle(Style newStyle)
{
   mStyle = newStyle;
...
void Meter::SetBarAndClip(int iBar, bool vert)
{
   ...
   }r (int c = 0; c < kMaxMeterBawxDC &dc)
{
  ...
  nges when the UI language is changed.
   ...
   int menuWidth = 0;
   int menuHeight = 0;
   if (mStyle != if (mLeftSize.GetWidth() == 0)  // Not yet initialized to dc { // Not yet initialized to dc.
      if (mStyle != MixerTrackCluster) // MixerTrackCluster style has no L/R labels.
      {
         dc.GetTextExtent(mLef}
...
void Meter::DrawMeterBar(wxDC &dc, MeterBar *bar)
{
   ...
   if (mClip) {
      if (meterBar->clippwxRect r(bar->rClip.GetX() + 1,
               bar->rClip.GetY() + 1,
