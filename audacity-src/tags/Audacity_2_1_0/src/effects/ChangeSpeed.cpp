...
// the standard vinyrpmPM choices
...
wxString EffectChangeSpeed::GetEffectDescription() { 
   ...
   return wxString::Format(_("Applied effect:3%s %.1f%%"), 
...
bool EffectChangeSpeed::PromptUser()
{
   ...
   dlog.mFromVinyl = mFromVinyl;FromLength = mFromLength;
   dlog.mTimeCtrlFormat = mTimeCtrlFormat= mToVinyl;
   ...
   mFromVinyl = dlog.mFroimeCtrlFormat = dlog.mTimeCtrlFormat;
...
bool EffectChangeSpeed::Init()
{
   ...
   mFromLength = mT1 - mT0entChange, 0.0);
   return true;
}
...
bool EffectChangeSpeed::Process()
{
   ...
   if (bGoodResult)
      ReplaceProcessedTracks(bG// Update selection.
   mT1 = mT0 + (((mT1 - mT0) * 100.0) / (100.0 + m_PercentChange));pdate selection.
...
#define PERCENTCHANGE_MAX 100 // warped above zero to actually go up to 40MAX_TEXT 4900 // maximum allowed by text entry (= 50 times)to actually go up to 400%
#define PERCENTCHANGE_SLIDER_WARP 1.30105 // warp power takes max from 100 to 400.

enum {
   ID_TEXT_PERCTEXT_MULTIPLIER,
   ID_SLIDER_PERCENTCHANGE,
   ID_CHOICE_FROMVINYL,
   ID_CHOICE_TOVINYL,
   ID_TIMECTRL_TOLENGTHNYL,
   ID_CHOICE_TOVINYL
};
...
BEGIN_EVENT_TABLE(ChangeSpeedDialog, EffectDialog)
    EVT_TEXT(ID_TEXT_PERCENTCHANGE, ChangeSpeedDialog::OnTexTEXT(ID_TEXT_MULTIPLIER, ChangeSpeedDialog::OnText_Multiplierlog::OnText_PercentChange)
   ...
   mEffect(effect)
{
   ...
   mToLength = 0.0;
   mFormat = wxT("");
   mTimeCtrlFormat = 0 mToVinyl = kVinyl_33AndAThird; 
...
void ChangeSpeedDialog::PopulateOrExchange(ShuttleGui & S)
{
   ...
   S.AddSpace(0, 5);
   S.AddTitle(_("Change Speed, affe   S.AddSpace(0, 10);
...
tMultiColumn(2, wxCENTER);
   {
      mpTextCtrl_PercentChange = 
         S.Id(ID_TEXT_PERCENTCHANGE).AddTextBox(_("PercentmpTextCtrl_PercentChange->SetValidator(validator);
   }
   S.EndMultiColumn();
   ...
   {
      /* i18n-hint: "rpm" is an English abbreviation meaning "revolutions per minute". */tartMultiColumn(5, wxCENTER);
   {
 rpm  S.AddUnits(_("Standard Vinyl RPM:"));
      ...
      mrpmoice_FromVinyl->SetName(_("From RPM"));
      mpChoice_FromVinyl->SetSizeHints(100, -1);

      mpChoice_ToVinyl = 
         S.Id(ID_CHOICE_TOVINYL).AddChoice(_("to"), wxT(""), &rpmStrings);
  rpm mpChoice_ToVinyl->SetName(_("To RPM"));
      mpChoice_ToVinyl->SetSizeHints(10
...
bool ChangeSpeedDialog::Validate()
{
   ...
   m_PercentChange = TrapDouble(m_PercentChange, PERCENTCHANGE_MIN, PERCENTCHANGE_MAX_TEXT);oVinyl = mpChoice_ToVinyl->GetSelection();
...
void ChangeSpeedDialog::OnText_PercentChange(wxCommandEvent & WXUNUSED(event)double newValue = 0;
...
void ChangeSpeedDialog::OnText_Multiplieroid ChangeSpeedDialog::OnText_PercentChange(wxCommandEvent & WXUNUSED(event)double newValue = 0;
   ...
   this->Update_Vinyl();
   this->Update_TimeCtrl_ToLength();
   mbLoopDetect = false;OK)->Enable(m_PercentChange > -100.0);
   }
}

void ChangeSpeedDialog::OnSlider_PercentChange(wxCommandEvent & WXUNUSED(event)m_PercentChange = (double)(mpSlider_PercentChange->GetValue());
...
te percent change controls for new Vinyl values.
{
...
void ChangeSpeedDialog::OnTimeCtrlUpdate(wxCommandEvent &evt)
{
   ...
   mpFromLengthCtrl->SetValue(mFromLength);ntChange();
      mbLoopDetect = false;
   }
}
...
_PercentChange;
      if (unwarped > 0.0)
         // Un-warp values above zero tunwarped = pow(m_PercentChange, (1.0 / PERCENTCHANGE_SLIDER_WARP));
...
// Update Vinyl controls from percent change.
{
   ...
   switch (ratio)
   {
      case 0: // toRPM is the same as fromRPM
         if (mFromVinyl != kVinyl_NA) {
   ((m_PercentChange == 0.0) && mpChoice_FromVinyl)
         mpChoice_ToVinyl->SetSele   } else {
