   ...
   if (!vstpath.empty())
   {
      ...
      while (tok.HasMoreTokens())
      {
         pathList.Add(tok.GetNextToken());
                     ...
                     ~/Library/Audio/Plug-Ins/VST and /Library/Audio/Plug-Ins/VST
...
// Our window event handler callback tascal OSStatus VSTEffectDialog::OverlayEventHandler(EventHandlerCallRef handler, EventRef event, void  *)data)->OnOverlayEvent(handler, event);
}

// Overlay window event handler
OSStatus VSTEffectndler
...
// Our window event handler callback t::Windowtatus VSTEffectDialog::OverlayEventHandler(EventHandlerCallRef handler, EventRef event, void 
{
...
intptr_t VSTEffect::mCurrentEffectID;  typedef AEffect *(*vstPluginMain)(audioMasterCallback audioMaster);
                            ...
                            float opt)
{
   ...
   // Handles operations during initialization...before VSTEffect hasmCurrentEffect set its instance pointer.
   switch (opcode)
   {
...
bool VSTEffect::RealtimeInitialize()
{
   ...
   return ProcessInitialize()n != bool VSTEffect::RealtimeAddProcessor(int numChannels, float sampleRate)
{
...
wxSizer *VSTEffectDialog::BuildProgramBar()
{
   ...
   // it wou
  ld not let me set aGetString(effGetProgramNameIndexed, i));
   }
   ...
   pathlog::OnLoad(wxCommandEvent & WXUNUSED(event))
{
  ...
  ctor(_("Load VST Preset:"),
   ...
   {
      // This shouldn't happen, but complain anyway
      wxMessageBox(_("Unrecognized file extensionffSetProgramName, fn, i)_("Error Loading VST Presets"),
                   wxOK | wxCENTRE,
                   mParent),
...
rogramName, fn, i);_("Error Loading VST Presets"),
                   wxOK | wxCENTRE,
                   mParent);
...
bool VSTEffectpleRate = 44100.0;
   mTimeInfo.tempo = 120.0;
   mTimeInfo.timeSigNulong effectID = 0;
   wxString realPath = mPath.BeforeFirst(wxT(';'));
   mPath.AfterFirst(wxT(';')).ToLong(&effectID);
   mCurrentEffectID = (intptr_t) effectIDeSigNumerator = 4;
...
bool VSTEffect::Load()
{
  ...
  le, "VSTPluginMain");
   ...
   // Open the resource mrealp ... some plugins (lik
      {
         return false;
      }
  ...
  )
{
  ...
  yEventHandlerUPP = 0;
   ...
   if (mWindowEventHandlerRef) {
      ::RemoveEventHandler(mWindowmDialog->EventHandlerRef);
      mWindowEventHandlerRef = 0;
      MacInstallTopLevel
  WindowEventHandler();
   }
...
void VSTEffectowEventHandlerUPP = 0;
   }
#endif
}
...
void VSTEffectDialog::BuildFancy()
{
   ...
   // Some effeco have us get their rect before opening them.
   ...
   // determine if another mDialog->window was opened above ours.
   ...
   wxBoxSizer *vs = new wxBoxSizer(wxVERT#if defined(__WXMAC__)
   ...
   // done last since proper window sizing will not  mDialog->MacGetEventHandler());
...
void VSTEffectif defined(__WXMSW__)
#else
#endif
}
...
void VSTEffectDialog::BuildPlain()
{
  ...
  Sizer *paramSizer = new wxStaticBoxSizer(wxVERTICAL, mParenttton), 0, wxEXPAND);

   SetSizer(vSizer);

   wxSizer *paramSizer = new wxStaticBoxSizer(wxVERTICAL, sw, _("Effect Settings"));Add the duration control for generators
 ...
 | wxALL, 5);
      ...
      mSliders[i] = new wxSlider(sw,
                                 ...
                                 wxDefaultPosition,
                                 wxSize(200, -1));
      gridSizer->AddmParentSliders[i], 0, wxALIGN_CENTER_VERTICAL | wxEXPAND | wxALL, 5);
...
T | wxALL, 5);
      ...
      gridSizer->Add(mLabels[i], 0, wxALIGN_CENTmParent->SetSizer(paramSizer);
...
ight(1) != wxT(':')) {
         ...
         mNames[i]-// a side effect that causes it to only move when the parameter changes
...
void VSTEffectiner->SetMinSize(evt.GetInt(), (int) evt.GetExtraLong());
   Fit();
   Layout();
}

void VSTEffectDialog::OnSliderSLIDERS;
