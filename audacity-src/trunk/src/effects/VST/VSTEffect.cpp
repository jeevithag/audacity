...
#i#if defined(__WXMAC__)
#include <dlfcn.h>
#include <wx/mac/private.h>
#elif defined(__WXMSW__)
#include <wx/dynlib.h>include <wx/msw/seh.h>
#include <shlwapi.h>
...
{), file.c_str()));
   ...
   deleteString::FromUTF8(getenv("VST_PATH"));
   if (!vstpath.empty())
   {
...
#if defined(__WXMAC__)  UniquePathToPathList(prefix + wxT("plugins"),
                                         pathList);
      wxGetApp().AddUniquePathToPathLpathList.Add(VSTPATH);
   ...
   // bundles.
   pm.FindFilesInPathList(wxT("Info.plist"), pathList, files, true"))) + VSTPATH,
                                      pathList);

   // Recursively search all paths for Info.pl
  ist files.  This will identify all
   // bundles.
   wxGetApp().FindFilesInPathList(wxT("Info.pl
     ist"), pathList, files, wxDIR_DEFAULT);

   // Remove the 'Contents/Info.plist' portion of the names
   for (size_t i = 0; i < files.GetCount(); i++) {
      files[i] = wxPathOnly(wxPathOnly(filesWXSIZEOF(tpathndsWith(wxT(".vst"))) {
         files.RemoveAt(i--);
      }
   }
   ...
   len = WXSIZEOF(tpathndsWith(wxT(".vst"))) {
         files.RemoveAt(i--);
      }
   }
   ...
   // Try HKEY_CURRENT_USER registry key TRUt
   len = sizeof(tpath) / sizeof(TCHAR);
   if (SHRegGetUSValue(wxT("Softw
   {
      tpath[len] = 0;
...
DEFINE_LOCALL_EVENT_TYPE(EVT_SIZEWINDOW);
DECLARE_LOCAL_EVENT_TYPE(EVT_UPDATEDISPLAY, -1);
DEFINE_LOCAL_EVENT_TYPE(EVT_UPDATEDISPLAY);

class VSTEffectDialog:publc wxDialog, XMLTagHandler
{
 public:
   VSTEffectDialog(wxWindow * parent,
                   ...
                   VSTEffect *effect,
                   AEffect *aeffect);
   virtual EnableApply(bool enable);

private:  virtual ~VSTEffectDialog();
...
private: 
   ...
   void RefreshP// Program/Bank loading/saving
   ...
   void SaveFXProgram(wxMemoryBuffer & buf, int indexefreshParameters(int skip = -1);

   virtual bool HandleXMLTag(const wxChar *tag, const wxChar **attrs);
   virtual void HandleXMLEndTag(const wxChar *tag);
   virtual void HandleXMLContent(const wxString & content);
   virtual XMLTagHandler *Handl
private:
   ...
   wxSizerItem *mContainer;xSizerItem *mContainer;
#endif
   ...
   WindowRef mPreviousBlocked;

   WindowRef mWindowRef;
   EventHandlerUPP mWindowEventHandlerUPP;
   EventHandle#elif defined(__WXMSW__)
...
static const EventTy#if !defined(EXPERIMENTAL_REALTIME_EFFECTS)
   { kEventClassWindow, kEventWindowGetClickModality },
#endif
#if 0ndow, kEventWindowActivated },
   ...
   { kEventClassMou#endif
};

// OverlayindowDeactivated },
};
...
{
   //nt of all the rest of thewindow in from of all the rest non-floaters.
   ...
   printf("WINDOW class %4.4s kind %d ewin %p owin %p mwin %p anf %p fnf %p\n",
      ...
      mOverlayRef,
      mWindowRef,
      ActiveNonFloatingWindow()bool block = false;
   WindowModality kind;
   WindowRef ref = NULL;
   ...
   {rlayEventHandlerRef);
      ...
      // curret top window.  Any other windows wouedld mean that a
      // modal dialog box has been open on top and we should block.
      cAnnounce the event may need blocking
         HIModalClickResult res = block ? kHIModalClickIsModal | kHIModalClickAnnounce : 0ity kind;
...
{
   { kGetClickModality },TypeSpec WindowEventList[] =
{
   { kEventClassWindow, kEventWindowShown },
   { kEventClas#if 0ndow, kEventWindowActivated },
   { kEventClassMouse,  kEventMouseDown },
   { kEventClassMouse,  kEventMouseUp },
   { kEventClassMouse,  kEventMouseMoved },
   { kEventClassMouse,  kEventMouseDragged },
   { kEventClassMouse,  kEventMouseEntered },
   { kEventClassMouse,  kEventMouseExited },
   { kEventClassMou#endifow, kEventWindowDeactivated },
};
...
OSStatus VSTEffectDialog::OnWindowEvent(EventHandlerCallRef handler, EventRef event)
{
   ...
   // Only kEventClassWindow events at this pointlassWindow event at this poine
   switch (GetEventKind(event))
   {
      // If we, Audacity will crash at  event Audacity, will crash attermination
      // since the window is still on the wxWidgets toplevel has already been deleted from the systembut
      // it's already gone.
  ...
  )
{
  ...
  WindowEventHandler();
   ...
   if (mWindowEventHandlerUPP) {
      DisposeEventHandlerUPP(mWindowElif defined(__WXMSW__)
   if (mHwnd)
   {
   ...
   if (mXwin)
   {
      ...
      mXdisp = NULL;
      mXwin = NULL(mWindowEventHandlerUPP);
      mWindowEventHandlerUPP = 0;
   }
#endif
}
...
void VSTEffectDialog::BuildFancy()
{
   ...
   mEffect->PowerOn()ruct
   {
      short top, left, bottom, right;
   } *rect;

   // Some effects like to have us get their rect before opening them.
   mEffect->callDispatcher(efEditGetRect, 0, 0, &rect, 0.0);
   ...
   HIViewRef subv
   {
      ...
      mGui = false;
      RemoveHandler();
      BuildPlPanel *w = new wxPanel(this, wxID_ANY);
   mHwnd = w->GetHWND();
   ...
   mEffect->callDispatcher(effEditOpen, 0, (intptr_t)mXdisp, (void *)mXwin, 0.0);llDispatcher(effEditOpen, 0, 0, w->GetHWND(), 0.0);
   ...
   // Add the program bar at the to#if defined(__WXMAC__)

   // Reserve space for the effect GUI/ Reserve space for the effect GUI
#if defined(__WXMAC__)

   mContainer = hs->Add(rect->right - rect->left,// Add the effect host window to the layouteft, rect->bottom - rect->top);

#elif defined(__WXMSW__)

   mContainer = hs->Add(w, 1, wxCENTER | wxEXPAND);
   mContainer->SetMinSize(rec   // Add the effect host window to the layouteft, rect->bottom - rect->top);

#elif defined(__WXMSW__)

   mContainer = hs->Add(w, 1, wxCENTER | wxEXPAND);
   mContainer->SetMinSizndif
...
#if defined(__WXMAC__)celButton|eOkButton), 0, wxEXPAND);
   SetSizerAndFit(vs);

   // Found out where the reserved sp pos = mContainer->GetPosition();
   ...
   ::RemoveEventHandler((EventHandlerRef)MacG
   mEffect->NeedEditIdle(true);GetEventHandler());
...
wxSizer *VSTEffectDialog::BuildProgramBar()
{
   ...
   // it wou
  ld not let me set a program outside of the first 128.
  ...
  0, 0, NULL, 0.0);
   ...
   hs->Add(bt, 0, wxALIGN_CENTER_VERTICAL | wxALI
   {
      for (int i = 0; i < mAEffect->numParams; i++)
      {
         wxString text = mEffect->GetString(effGetParamName, i);
        ...
        Name, i).Trim(true).Trim(false);
         ...
         // from off to on.  However, this prevents changin
        g the value using the
...
void VSTEffectDialog::OnUpdateDisplay(wxCommandEvent & evt)
{
   ...
   // Refresh the program list since some effects change the available programs
   based
   ...
   // The new list may not have the previously selected program or the user 
  may have
   // changed it
   i = mEffect->callDispatcher(effGetProgram, 0, 0, NULL, 0.0);
   if (i >= 0) {
      mProgram->SetSelectio
  n(i);
   }
...
{
   int i = m
  Effect->callDispatcher(effGetProgram, 0, 0, NULL, 0.0);
   ...
   int ip = mProgram-length() > 24)
   {
      name = name.substr(0, ength of the string, max 24 + 1 for null terminator
   if (name.Length() > 24) {
      name = name.Left(24);
   }
...
void VSTEffectDialog::OnLoad(wxCommandEvent & WXUNUSED(evt))
{
   ...
   pathlog::OnLoad(wxCommandEvent & WXUNUSED(event))
{
   wxString fn;

   // Ask the user for the real name
   fn = FileSelector(_("Load VST Preset:"),
                     FileN  wxT("VST preset files (*.fxb; *.fxp; *.xml)|*.fxb; wxEmptyString,
                     wxT("xml"),
  ...
  path.IsEmpty())
   ...
   else
   {
      ...
      wxMessageBox(_("Unrecognized file extensionffSetProgramName, fn, i);
      }
...
bool VSTEffectDialog::LoadFXB(const wxFileName & fn)
{
   ...
   if (!data)
   {
      wxMessageBox(_("Unable to allocate memory when loading presets fileffSetProgramName, fn, i);
      }
...
ogramName, fn, i);
  ...
  ProgramName, fn, i);
      ...
      if (error) {
         wxMessageBox(_("Co  wxOK | wxCENTRE,
                        this);
         break;
      }
...
void VSTEffectDialog::SaveXML(const wxFileName & fn)
{
  ...
  xmlFile.WriteAttr(wxT("name"),
                              mEffect->GetString(mEffect->callGetParameter(i)));
...
void VSTEffectDialog::OnOk(wxCommandEvent & WXUNUSED(ev      mEffect->callDispatcher(effEditClose, 0, 0, NULL, 0.0);
   }
...
// builds
   ...
   if (mGui)       mEffect->callDispatcher(effEditClose, 0, 0, NULL, 0.0);
   }
...
void VSTEffectDialog::OnDefaults(wxCommandEvent & WXUNUSED(evt))
{
   ...
   RefreshParameters(event a brief empty dialog
   Show(false);
#endif
   ...
   if (wxStrcmp(tag, wxT("para
            
   if (wxStrcmp(tag, wxT("program")) == 0) {
      while (*attrs) {
         const wxChar *attr = *attrs+version")) == 0)
         {
      ...
      while (*attrs) {
         ...
         if (wxStrcmpName())
            ...
            if (value != mEffect->GetEffectIdentifier()) {
               wxString msg;
               msg.Printf(_("This 
              parameter file was saved from %s.  Continue?"), value);
               int result = wxMessageBox(msg, wxT("Confirm"), wxYES_NO, thi
         {
            if (!XMLValueChecker::IsGoodInt(strValue))
             // tell the user
               return false;
   ...
   if (wxStrcmp(tag, wxT("program")) == 0) {
      while (*attrs) {
         ...
         c
         {
            ...
            mEffect->SetString(effSetProgramN
  ame, strValue, ndx);
         }
         else {
            return fa
   ...
   if (wxStrcmp(tag, wxT("program")) == 0) {
      while (*attrs) {
         const wxChar *attr = *attrs+index")) == 0)
         {
        ...
        ecker::IsGoodString(strValue)) {
            ...
            if (val < 0.0 || val > 1.0)
           ValueChecker::IsGoodInt(strValue) ||
   ...
   if (wxStrcmp(tag, wxT("
   {
      if (mChunk.length())
      {
         ...
         if (len)
         (wxStrcmp(tag, wxT("chunk")) == 0) {
      if (mChunk.Length()) {
         char *buf = new char[mChunk.Length() / 4 * 3];
c         int len = b64decode(mChunk, buf);
         if (len) {
                            ...
                            float opt)
{
   ...
   switch (opcode)
   {
      ...
      case audioMasterGetVendorVersion:
         return (intptr_t) (AUour case) is connected
      ...
      // Some (older) effects depend on an effIdle call when requested.  An
         ...
         {
            vst->NeedIdle();
            return 1lugins which uses the call to update
...
bool VSTEffect::ProcessInitialize()
{
   ...
   memset(&mTimeInfo, 0, sizeof(mTimeInfo));
   mTimeInfo.sampleRate = mSampleRate;         // in GUI thread

   memset(&mTimeInfo, 0, sizeof(mTimeInfo));
   mTimeInfo.samplePos = 0.0;
   mTimeInfo.sampleRate = 44100.0;  // this is a bogus value, but it's only for the display
   mTimeInfo.nanoSeco | kVstTransportPlaying;
...
//
// And at the same time I added buffer provide the information (kn0ck0ut is one).
//
...
bool VSTEffect::Load()
{
  ...
  e(urlRef);
   ...
   // Bail
   if we couldn't resolve the executable path
   if (good == FALSE) {
      CFRelease(bundleRef);
      return false;
   }
  ...
  se(bundleRef);
   ...
   pluginMain = (vstPluginMain) dlsym(mModu
  le, "VSTPluginMain");
   ...
   // Need to keepif defined(__WXMSW__)he bundle reference around so we can map the
   // resources.
   mBundleRef = bundleRef;

   // Open the resource map ... some plugins (lik
      {
         return false;
      }
...
int VSTEffect::GetString(wxString & outstr, int opcode, int index)
{
   ...
   if (ret) {
      outstr = LAT1CTOW   strcpy(buf, str.Left(255).ToUTF8ring VSTEffect::GetString(int opcode, int index)
{
   wxString str;
...
void VSTEffect::SetString(int opcode, const wxString & str, int index)// Needed since we might be in the dispatcher when the timer pops
   wxCRIT_SECT_LOCKER(locker, mDispatcherLock);
   return mAEffect->dispatcher(mAEffect, opcode, index, value, ptr, optvalue, void *ptr, float opt)
{
