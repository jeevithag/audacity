...
void AudioUnitEffectsModule::LoadAudioUnitsOfType(OSType inAUType,
                                                  wxArrayString & effectsdioUnitEffect::GetCarbonViewComponent(OSType subtype)
{
   ComponentDescription desinAUType;
...
void AudioUnitEffectImportDialog::OnOk(wxCommandEvent & WXUNUSED(evt))
{
   ...
   while ((sel = mList->GetNextItem(sel, wxLIST_NEXT_ALL, wxLIST_STATE_SELECTED)) >= 0)
   {
      ...
      OSStatus result = AudioUnitSetProperty(mEffect->mUnit,
                                             kAudioUnitProperty_ClassInfo,
        AudioUnitProperty_MaximumFramesPerSlice,
                                      kAudioUnitScope_Glo        0,
                                             &content,
                                             sizeof(content));
      CFRelease(content);
...
AudioUnitProperty_MaximumFramesPerSlice,
   ...
   result = AudioUnitGetProperty(mUnit,
                                 kAudioUnitProperty_ParameterList,
oUnitProperty_MaximumFramesPerSlice,
                                      kAudioUnitScope0,
                                 array,
                                 &dataSize);  
   if (result != noErr)
...
udioUnitProperty_MaximumFramesPerSlice,
                                      kAudioUnitScope_Glarray[i],
                                    &info,
                                    &dataSize);  
      if (result != noErr)
      {
...
erty_MaximumFramesPerSlice,
   ...
   mBlockSize = 512;
   AudioUnitGetProperty(mUnit,
oUnitSetProperty(trackUnit,
                                      kAudioUnitProperty_MaximkAudioUnitScope_Global,
                        0,
                        &mBlockSize,
                        &dataSize);  
...
bool AudioUnitEffect::ProcessInitialize()
{
   ...
   mT(&timeStamp, 0, sizeof(AudioTimeStamp));
   timeStamp.mSampleTime = 0; // This is a double-precision numb // accumulate the number of frames processed so far
   ...
   callbackStruct.inputProcRefComhis;
  ...
  itialize(trackUnit);
   if (auResult != 0) {   
      ...
      retumUnit, kAudioUnitScope_Global, 0);
   if (auResult != 0)
   {
      return false;
   }
...
lock, bufferList);
      if (auResult != 0) {
         printf("Render failed: %d %4return 0;
   }
...
bool AudioUnitEffect::PopulateUI(wxWindow *parent)
{
   ...
   mEventRef = GetControlEventTarget(auViewbe
   // done last since proper window sizing will not occur otherwise.
   ...
   // it's not captured, we will crash at Audacity terminatioAudioUnitEffect::WindowEventHandlerCallbackwindow
   // is still on the wxWidgets toplevel window lists, but it's already gone.
   mHandlerUPP = NewEventHandlerUPP(EventHandlewindowEventList),
                             windowE,
                             mHandlerUPP,
                             GetEventTypeCount/* Was intended for improved resizing...not being used
   // done last since proper window sizing will not occur otherwise.
   ::RemoveEventHandler((EventHandlerRef)MacGetEventHandler());

   // Install a bare minimum handler so we can capture the window close event.  If
   // it's not captured, we wControlHandlerUPP = NewEventHandlerUPP(AudioUnitEffect::ControlEventHandlerCallback);
...
dioUnitProperty_MaximumFramesPerSlice,
                                      kAudioUnitScope_G0,
                                   &mSampleRate,
                                   sizeof(Float64));
   if (auResult != 0)
...
mAudioInormat.mChannelsPerFrame = numChannels;
   ...
   auResult = AudioUnitSetProperty(mUnit,
            ...
            Output,
                                                   kAudioUnitScope_Input, 0,
                                   &streamFormat,
                                   sizeof(AudioStreamBasicDe{Didn't accept global stream format\n");
      return false;
   }
...
dioUnitProperty_MaximumFramesPerSlice,
  ...
  rameters, &size);
   ...
   // Copy the parameters from the main unit to the (i = 0; i < numParameters; i++)
   {
                                    ...
                                    AudioUnitParameterValue inParameterValue)
{
   ...
   if (inEvent->mEventType == kAudioUnitEvent_PropertyChange)
   {
      ...
      if (inEvent->mArgument.mProperty.mPropertyID == kAudioUnitProperty_Latency)
      {
         ...
         AudioUnitGetProperty(mUnit,
                              kAudioUnitProperty_Latency,
itProperty_MaximumFramesPerSlice,
                                      kAudioUnitSc0,
                              &mLatency,
                              &dataSize);  
...
Property_MaximumFramesPerSlice,
                                      kAudioUnit0,
                            inParameterValue,
                            0);
   }
