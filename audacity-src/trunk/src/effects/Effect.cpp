...
#include <wx/timer.h>
#include <wxtglbtnr.h>
#include <wx/hashmap.h>#include <wx/utilsp.h>
...
#include "../Project.h"
#include "../WaveTrack.h"
#include "..toolbars/ControlToolBark.h"
#include "../widgetsAButtonk.h"
...
bool Effect:Progress(int whichTrack, double frac, wxString msg)
{
...
void Effect::GetSamples(WaveTrack *track, sampleCount *start, sampleCount *len)
{
   ...
   double t1 = mT1 > trackEnd ? trackEnd : #if 0
   if (GetType() & INSERT_EFFECT) {
      t1 = t0 + mDurationLength;
      if (mT0 == mT1) {
...
void Effect::CopyInputTracks(int trackType)
{
   ...
   for (Track *aTrack = aIt.First(); aTrack; aTrack = aIt.
   {t()) {
...
{
   retbool Effect::SupportsRealtime()
{
...
{
   return mClient->RealtimeProcessEnd(); retbool Effect::IsRealtimeActive()
{
...
void Effect::Preview(bool dryOnly)
{
 ...
 mProgress = NULLess();
   ...
   delete mTracksint Effect::GetAudioInCount()
{
   if (mClient)
   {
