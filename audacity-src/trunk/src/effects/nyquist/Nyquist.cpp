...
#include <wx/choice.h>#include <wx/checkboxe.h>

#f defined EXPERIMENTAL_NYQUIST_TIME_PROPERTY
...
#include "../../WaveClip.h"
#include "../../WaveTracki.h"
#include "../../widgets/valnum.h"#include "../../Prefsm.h"
...
EffectNyquist::EffectNyquist(wxString fName)
{
   ...
   mDebug = false;
   mIsSal = false;
   mOK = false;   mAuthor = wxT("N/A");
   mCopyright = wxT("N/A")e;   // set clip/split handling when applying over clip boundary.
   ...
   if (fName == wxT("")) {
      // Interactive Nyquist
     gPrefs->Read(wxT("/Effects/NyquistPrompt/Version"), &mVersion, 4);t
      mOK = true;
      mInteractive = true;
...
bool EffectNNyquist::TransferParameters( Shuttle & shuttle )
{
...
bool EffectNyquist::PromptUser()
{
   if (mInteractive) {
      NyquistInputDialog dlog(wxGetTopLevelParent(NULL), -1,
                              _("Nyquist Prompt"),
                              _("Enter NyqmVersion = mVersion _("Enter Nyquist Command: "),
                              mInputCmd);
      dlog.CentreOnParent();
      int result = dlog.ShowModal();

  mVersion = dlog.mVersion;l();
      ...
      Preview()wxOK | wxCENTRE);
            retur//TODO: If we want to auto-add parameters from spectral selection,
...
bool EffectNyquist::Process()
{
   ...
   mProgressOut = 0;
   mProgressTot = 0;
   mScale = (GetEffectFlags() & PROCESS_EFFECT ? 0.5 TrackIndex = 0;
...
arator when giving
         // numbers to Nyquist, whereas using "%f" wilmProps += wxString::Format(wxT("(putprop '*PROJECT* (float %s) 'RATE)\n"),
   ...
   while (mCurTrack[0]) {
     ...
     String(wxT("C")xString prevlocale = wxSetlocale(LC_NUMERIC, NULL);
   ...
   for (unsigned int j = 0; j < mControls.GetCount(); j++) {
      if (mControls[j].type == NYQ_CTRL_REAL) {
         ...
         cmd += wxString::Format(wxT("(setf %s %d)\n"),
                                 mControls[j].var.c_str(),
                                 (int  str.Replace(wxT("\\"), wxT("\\\\"));
         str.Replace(wxT("\""), wxT("\\\""));
         cmd += wxT("(setf ");
         // restrict variable names to 7-bitEscapeString(mControls[j].valStr)CII:
         cmd += mControls[j].var.c_str();
         cmd += wxT(" \"");
         cmd += str; // unrestricted value will become quoted UTF-8
EscapeString(strtr = mCmd;
      str.Replace(wxT("\\"), wxT("\\\\"));
      str.Replace(wxT("\""), wxT("\\\""));
...
// PRL:  to do:
         ...
         ltrack->AddLabel(t0 // This should not happen, but leaving in for now just in case (Dec 2014)
      wxMessageBox(_("Undefined return value.\n")ed the value:") + wxString(wxT(" %d")),
                 nyx_get_int());
    id not return audio.\n"), wxT("Nyquist"),
                   wxOK | wxCENTRE, mParent);
      return false;
   ...
   for (i = 0; i < mCurNumChannels; i++) {
      ...
      if (outChannels == mCurNumCh      if (mMergeClips < 0) {
...
void EffectNyquist::Parse(wxString line)
{
   ...
   // (Version 3 added support for choice pa// (Version 4 added support for project/track/selection information.)
...
2 && tokens[0] == wxT("author")) {
      mAuthor
      mAction = UnQuote(tokens[1]);
      return;
   }

   if (len >= 2 && tokecopyright")) {
      mCopyright = UnQuote(tokens[1]);te(tokens[1]);
      return;
   }
...
bool EffectNyquist::SetXlispPath()
{
   ...
   fname = mXlispPath + wxFILE_SEP_PATH + wxT("utChannels; i++) {
      delete mOutputTrack[i];
      mOutputTrack[i] = NULL;
   }
...
void EffectNyquist::OtputCallback(int c)
{
   if (mDebug && !mExternal) {
      mDebugOutput += (char)c;
      return;
...
void EffectNyquist::OSCallback()
{
   ...
   wxYieldIfNeeded();
#endifreakwxArrayString EffectNyquist::GetNyquistSearchPath()
{
   wxArrayString audacityPathList = wxGetApp().audacityPathList;
   wxArrayString pathList;
   ...
   {
i = 0; i < audacityPathList.GetCount(); i++) {
         wxString prefwxGetApp().AddUniquePathToPathList(prefix + wxT("nyquist"), pathList);
...
{
   mEffect->Preview(tDialog//////////////////////////////////////////////////////////////////////////////
//
// NyquistInputDialog
//
///////////////////////////////////////////////////////////////////////////////
_#define ID_VERSION 1001

BEGIN_EVENT_TABLE(NyquistInputDialog, wxDialog)
   EVT_CHECKBOX(ID_VERSION, NyquistInputDialog::OnVersionCheckndEvent & /* event */)
{
   // Transfer data
                                       ...
                                       c, wxDefaultPosition, wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER)
{
   ...
   GetSizer()ainSizer->Add(CreateStdButtonSizer(this, eDebugButton|e// ============================================================================
// NyquistInputDialog implementation
// ============================================================================
 eCancelButton|eOkButton), 0, wxEXPAND);

   SetAutoLayout(true);
   SetSizer(mainSizer);
   mainSizer->Fit(thisVersionCheck(wxCommandEvent& WXUNUSED(evt))
{
   mVersion = (mVersionCheckBox->GetValue()) ? 3 : 4;
   gPrefs->Write(wxT("/Effects/NyquistPrompt/Version"), mVersion);
   gPrefs->FlushmainSizer);
   mainSizer->Fit(this);
   mainSizer->SetSizeHints(this);
...
// ============================================================================izer->Add(hSizer, 0, wxALIGN_CENTRE | wxLEFT | wxBOTTOM | wxRIGHT, 5);
