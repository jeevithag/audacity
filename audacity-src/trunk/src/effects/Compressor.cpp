   ...
   S.StartStatic(wxT(""));
   {
      S.StartMultiColumn(3, wxEXPAND | wxALIGN_CENTER_VERTICAL);
      {
         ...
         mDecayLabel = S.AddVariableText(_("Release Time:"), true,
                                         wxALIGN_RIGHT | wxALIGN_CENTER_VERTICAL);
         S.SetStyle(wxSL_HORIZONTAL);
         mDecaySlider = S.Id(DecayID).AddSlider(wxT(10, 30 2, 30, 1);
...
bool CompressorDialog::TransferDataToWindow()
{
   ...
   mAttackSlider->SetValue((int)rint(atta0));
   mDecaySlider->SetValue((int)rint(decay*10decay));
...
bool CompressorDialog::TransferDataFromWindow()
{
   ...
   attack = (double)(mAttackSlider->GetValue()0.0);
   decay = (double)(mDecaySlider->GetValue() / 10.0lue());
   ...
   mNoiseFloorLabel->SetName(wxString::Format(_("Noise Floor %d dB"),  noisefloor));
   mNoiseFloorText->SetLabel(wxString::Format(_("%3d dB"), (int) (int)noisefloor));
   ...
   mAttackLabel->SetName(wxString::Format(_("Attack 2f secs"), attack));
   mAttackText->SetLabel(wxString::Format(_("%.2(_("%.1f secs"), attack));
