     ...
     wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER | wxMAXIMIZE_BOX )
{
   ...
   S.StartStatic( _("Parameters") );
   {
      S.StartMultiColumn(5, wxEXPAND);
      {
         ...
         NumericTextCtrl(NumericConverter::TIME, tCtrl(this,
                         ID_FOREGROUNDSTART_T,
...
{
   SetStartTime(mForegroundStartValue());
   SetEndTime(mForegroundEndT->GetetTimeValue());
   foregrounddB = GetDB();
...
{
   SetStartTime(mBackgroundStartValue());
   SetEndTime(mBackgroundEndT->GetetTimeValue());
   backgrounddB = GetDB();
...
void ContrastDialog::OnUseSelectionF(wxCommandEvent & event)
{
   ...
   while (t) {
      if (t->GetSelected() && t->GetKind() == Track::Wave) {
         mForegroundStartetTimeValue(p->mViewInectedRegion.t0());
         mForegroundEndT->SetValue(p->mViewInfo.selectedRegion.t1()o.sel1);
         break;
      }
      t = iter.Next();
   }
...
void ContrastDialog::OnUseSelectionB(wxCommandEvent & event)
{
   ...
   while (t) {
      if (t->GetSelected() && t->GetKind() == Track::Wave) {
         mBackgroundStartValue(p->mViewInfo.selectedRegion.t0());
         mBackgroundEndT->SetValue(p->mViewInfo.selectedRegion.t1()o.sel1);
         break;
      }
      t = iter.Next();
   }
...
void ContrastDialog::OnExport(wxCommandEvent & WXUNUSED(event))
{
   ...
   fName = FileSelector(_("Export Contrast Result As:"),
                  wxEmptyString  NULL, fName, wxT("txt"), wxT("*.txt"), wxFD_SAVE | wxRESIZE_BORDER, this);
   ...
   float t = (float)mForegroundStartetTimeValue();
   ...
   t = (float)mForegroundEndetTimeValue();
   h = (int)(t/3600);
   m = (int)((t - h*3600)/60);
   s = t - h*3600.0 - m*60.0;
   ...
   t = (float)mBackgroundStartetTimeValue();
   h = (int)(t/3600);
   m = (int)((t - h*3600)/60);
   s = t - h*3600.0 - m*60.0;
   f.AddLine(wxString::Format(_("Time started = %2d hour(s), %2d minute(s), %.2f seconds."), h, m, s ));
   t = (float)mBackgroundEndetTimeValue();
   h = (int)(t/3600);
   m = (int)((t - h*3600)/60);
   s = t - h*3600.0 - m*60.0;
   f.AddLine(wxString::Format(_("Time ended = %2d hour(s), %2d minute(s), %.2f seconds."), h, m, s ));
...
void ContrastDialog::OnReset(wxCommandEvent & event)
{
   ...
   mForegroundStarValue(0.0);
   mForegroundEndT->SetValue(0.0);
   mBackgroundStartT->SetValue(0.0);
   mBackgroundEndT->SetSetTimeValue(0.0);
