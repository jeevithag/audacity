...
void ImageRoll::DrawBitmap(wxDC &dc, wxBitmap &bitmap,
           wxRasterOperationMode             int x, int y, int logicalFunc)
{
...
void ImageRollwxRasterOperationModeraw(wxDC &dc, wxRect rect, int WXUNUSED(logicalFunc))
{
...
ImageRollPanel::ImageRollPanel(wxWindow *parent,
                               ...
                               const wxSize& size,
                               long style):
   wxPanel(parent, id, pos, size, stwxRasterOperationMode),
   mImageRoll(imgRoll),
...
void ImageRollPanel::OnPaint(wxPaintEvent & W