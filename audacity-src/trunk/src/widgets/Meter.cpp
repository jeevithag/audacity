...
#include "../Experimental.h#include "../widgets/valnum.h"
...
BEGIN_EVENT_TABLE(Meter, wxPanel)
   EVCOMMAND(wxID_ANY, EVT_AUDIOIO_MONITOR, Meter::OnAudioIOMonitorel)
...
wxColour backgroundColour =
  ...
  if (mStyle != MixerTrackCluster)
         mMeterDisabled = 1L;// Monitoring off by default.t) {
      ...
      mClipBrush = wxBrush( theTheme.Colour( clrMeterInputClipBrush   ), wxSOLI//      mLightPen  = wxPen(   theTheme.Colour( clrMeterInputLightPen    ), 1, wxSOLID);
//D);
      ...
      mLightPen  = wxP//      mDarkPen   = wxPen(   theTheme.Colour( clrMeterOutputDarkPen    ), 1, wxSOLID);
   }
...
void Meter::CreateIcon(int WXUNUSED(aquaOffset))
{
   ...
   {
      mIcon = new wxBitmapRegister for AudioIO Monitor events
   wxTheApp->Disconnect(EVT_AUDIOIO_MONITOR,
                        wxCommandEventHandler(Meter::OnAudioIOMonitor),
                        NULL,
                        this);
xBitmap(theTheme.Image( bmpSpeaker ));
   }
}
...
void Meter::UpdatePrefs()
{
   ...
   // MixerTrackCluster style has no menu, so disall{
      mMeterDisabled = 0L;
   }
   else
   {
   ...
   mLayoutValid = false;
   Refresh(trueDisabled = gPrefs->Read(wxT("/Meter/MeterOutputDisabled"), (long)0);
}

void Meter::OnErase(wxEraseEvent & WXUNUSED(event))
{
   // Ignore it to prevent flamLayoutValid = false;
...
ageBox(wxString::Format(" mHeight=%d, mWidth=%d", mHeight,mWidth));
   mLayoutValid = false;
}
...
void Meter::OnMouse(wxMouseEvent &evt)
{
       ...
       (evt.ButtonDown() && mMenuRect.Contains(evt.m_x, evt.m_y))) 
   {
      ...
      if (mIsInput) {
         if (gAudioIO->IsMonitoring())
            ...
            if (gAudioIO->AILAIsActive())
             Recordingu->Append(OnAutomatedInputLevelAdjustmentID, _("Stop Automated Input Level Adjustment"));
            else
              Recording->Append(OnAutomatedInputLevelAdjustmentID, _("Start Automated Input Level Adjustment"));
            ...
            if (!AVActive || !GetActiveProject()->GetControlToolBar()->IsRecordDown())
               menu->Enable(OnAuto//matedInputLevelAdjustmentID, fal//menu->Append(OnHorizontalID, _("Horizontal Stereo"));
      //
      ...
      //menu->Append(OnWaveformID, _("Wavef//menu->Enable(mStyle==VerticalStereo? OnVerticalID: OnHorizontalID, false);
      ...
      //menu->App      //menu->AppendSeparator();
ppend(OnFloatID, _("Float Window"));
...
{
   for (int c = 0; c SetBarClip( int iBar ) 
{
  ...
  nges when the UI language is changed.
   ...
   if (mLeftSize.x == 0) { // Not yet initialized to dc.
      ...
      if ((mStyle == VerticalStereo) || (mStyle == MixerTrackCluster)) // VerticalMulti?
      {
         ...
         int rulerWidth;
         int rulerHeight;
         dc.GetTextExtent(wxT("-888"), &rulerWidth, &rulerHeight); // -888
...
void Meter::HandleLayout()
{
   ...
   case MixerTrackCluster:
      ...
      mRuler.SetOrientation + 1,
                       mBar[1].r.x + width,
                       mBar[1].r.y + mBar[1].r.height - 1[1].r.y,
                       mBar[1].r.x + width,
   ...
   case HorizontalStereo:
      ...
      mRightTextPos = wxPoint(2, (r = wxRect(left+2, height/2 - barh - 1, barw, barh);
      SetBarClip( 0 );
   ...
   if (mStyle estDC)
{
   ...
   if (mStyle != MixerTrackCluster)
   {
      dc.Dra   // The style and sizing matches the ones in the title
...
PARENT_BRUSH);
      AColor::Bevel(   bool start = !mMonitoring;(dc, false, meterBar->rClip);
   }{
      gAudioIO->StopStream();
      if (!mMeterDisabled){
...
void Meter::OnPreferences(wxCommandEvent & WXUNUSED(event))
{
   wxNumberEntryDialog
      d(this,
        _("Higher refresh rates make the meter show);
