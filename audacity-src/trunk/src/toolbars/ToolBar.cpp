...
//
void ToolBarResizeoid ToolBar::OnErase( wxEraseEvent & WXUNUSED(event) )
{
...
//
voResizeoid ToolBar::OnPaint( wxPaintEvent & event )
{
...
void ToolBar::OnLeftDown( wxMouseEvent & event )
{
   ...
   event.SkRetrieve the mouse position
...
void ToolBarResizer::OnLeftUpr::OnLeftDown( wxMouseEvent & event )
{
   // Go ahead and set the event to propagate
   eventif( HasCapture() )
   {
...
void ToolBarResizer::OnEnterr::OnLeftDown( wxMouseEvent & event )
{
   // Go ahead and set the event to propagate
   eventif( !HasCapture() )
   {
...
void ToolBarResizer::OnLeaver::OnLeftDown( wxMouseEvent & event )
{
   // Go ahead and set the event to propagate
   eventif( !HasCapture() )
   {
...
void ToolBarResizer::OnMotioar::OnLeftDown( wxMouseEvent & event )
{
   // Go ahead and set the event to propagate
   event.Sk
   }
   ...
   wxPoint pos = ClientToScreen( raw_poevent.Dragging() )
   {
      wxRect r = mBar->GetRect();
      wxSize msz = mBar->GetMinSize();
      wxSize psz = mBar-> wxSize psz = GetParent()->GetClientSize();
...
: wxPanel()
{
   ...
   mSpacer = NULL;
   mDock = NULL;
   mVisible = fa   mPositionedsible = fa
   mGrabber = NULL;
   mResizer = NULLfa
   SetId(mType)false;
}
...
wxString ToolBar::GetTitle()
{
   ...
   return wxString::Format( _("Audacity %sbToolBar"), GetLabel().c_str() );
}
...
void ToolBar::ReCreateButtons()
{
 ...
 // Create the resizer and add it to the main sizer
      ...
      mResizer->SetToolTip( _("Click and drag to resize toolbar")TH, 1 );
   }
...
{
#if wxUSE_TOOLTIPS
   // Change the tooltip of the grabbeif ( mGrabber )
   {
