...
#includeSpectral "SelectionBar.h"
...
// SetWindowClass((WindowRef)d.MacGetWindowRef(), kFloatingWindowClass);
      ...
      SetSize( width + 2, bar->GDockedinSize().y + 2 );
...
: wxEvtHandler()
{
   ...
   mBars[ TransportBarID ]     = new ControlToolBar();RecordMeterBarID ]   = new MeterToolBar( parent, RecordMeterBarID );
   mBars[ PlayMeterBarID ]     = new MeterToolBar( parent, PlayMeterBarID );
   mBars[ MeterBarID ]         = new MeterToolBar( parent, MeterBarID terToolBar();
   ...
   mBars[ DeviceBarID ]        = new Device#ifdef EXPERIMENTAL_SPECTRAL_EDITING
...
GetSection() );
      ...
      if( dock )
      {
         ...
         {
            bar->Create( mBotDockint( x, y ) );

         // Set the width and height
         if( width[ ndx ] != -1 && height[ ndx ] != -1 )
         {
            wxSize sz( width[ ndx ], height[ ndx ]bar->SetSize( szCreate( mBotDock );
         }
...
void ToolManager::OnMouse( wxMouseEvent & event )
{
   ...
   else if( event.Dragging() && pos != mLastPos )
   {
      ...
      br.SetPosition( mBotDock->GetParent()->ClientToScreen( br.G
  ...
  Top    if( r.GetBottom() >= dr.GetHeight() )
            {
