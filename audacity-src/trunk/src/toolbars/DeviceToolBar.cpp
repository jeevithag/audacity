...
void DeviceToolBar::Create(wxWindow *parent)
{
   ...
   // Simulate a size event to set initial meter placement/size#if wxCHECK_VERSION(3, 0, 0)
...
void DeviceToolBar::Populate()
{
   ...
   mInput = new wxChoice(this,
                         ...
                         wxDefaultPosition,
                         wxDefaultSize);
   /* i18n-hint: (noun) It's the device userecording.*/
   mInput->SetName(_("Recording"Input Device"));
...
void DeviceToolBar::FillHosts()
{
   ...
   mHost->SetMaxSize(mHost->GetBestSize()etCount() == 0)
      mHost->Enable(false);
}
...
void DeviceToolBar::FillHostDevices()
{
   ...
   if (foundHostIndex == -1) {
      if (outMaps.size())
         ...
         foundMake sure in/out are clear in case no host was found
   mInput->Clear();
   mOutput->Clear();   foundHostIndex = inMaps[0].hostIndex;
   }
   ...
   // Repopulate the Inputavailable to the user
   mInput->Clear();
   for (i = 0; i < inMaps.size(); i++) {
   ...
   mInput->EnaInput->InvalidateBestSize();
   mInput->SetMaxSize(mInput->GetBestSize()put->En() ? true : false);
   ...
   mOutput->EnamOutput->InvalidateBestSize();
   mOutput->SetMaxSize(mOutput->GetBestSize()ut->Enable(mOutput->GetCount() ? true : false);
...
void DeviceToolBar::RecordingnputDialog()
{
   ShowComboDialog(mInput, wxString(_("Select Input Device")));
}
void DeviceToolBar::ShPlaybackutDialog()
{
...
void DeviceToolBar::ShowChannelRecordingog()
{
