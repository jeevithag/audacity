...
void MeterToolBar::RegenerateTooltips()
{
   ...
   mRecordMeter->SetToolTip( _("Recordingt Level (Click to monitor.)") );
#endif
}
...
//   width -= mSizer->GetPosition().x;
   ...
   // Default location forplayd meter
   ...
   if( width > height )
   {
      ...
      {
         // Stacked
         RecordMeter->SetMinSize( wxSize( width, ( height / 2 ) ) );d
   ...
   mSizer->SetItemPosition( mPlaydMeter, pos );
